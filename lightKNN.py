...
if __name__=="__main__":
    x = pd.read_json('res/train.json')
    X = pd.read_json('res/val.json')
    knn = LightKNN(k=100, rho=0.4, alpha=0.5, beta=0.5,\
                   sim_songs='idf', sim_tags='cosine', sim_normalize=False, verbose=True)
    knn.fit(x)
    pred = knn.predict(X)
    print(pred)
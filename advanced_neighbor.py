...
class Neighbor:
    ...
    def __init__(self, pow_alpha, pow_beta, displacement, train=None, val=None, song_meta=None, \
        ...
        if not (0 <= self.displacement <= 1):
            raise ValueError('displacement is out of [0,1].')
    ...
    def predict(self, start=0, end=None, auto_save=False, auto_save_step=500, auto_save_fname='auto_save'):
        ...
        pred = []
        all_songs = [songs for songs in self.train_songs]  # list of set
        all_tags  = [tags  for tags  in self.train_tags ]  # list of set
        ...
        for uth in _range:
            ...
            playlist_songs = self.val_songs[uth]
            playlist_tags = self.val_tags[uth]
            ...
            if playlist_size_songs != 0:
                ...
                for vth, vplaylist in enumerate(all_songs):
                    intersect = self._sim_playlists(playlist_songs, vplaylist)
            ...
            if playlist_size_tags != 0:
                ...
                for vth, vplaylist in enumerate(all_tags):
                    intersect = self._sim_playlists(playlist_tags, vplaylist)
                    weight = 1 / (pow(len(vplaylist), self.pow_alpha))
                    if intersect != 0:
                        for track_i in vplaylist:
                            track_feature[track_i][vth] = intersect * weight
    ...
    def _sim_playlists(self, playlist_u, playlist_v):
        '''
        playlist_u, playlist_v : list
        '''
        sim = 0
        for element in playlist_u:
            if element in playlist_v:
                pos_u = (playlist_u.index(element) + self.displacement) / len(playlist_u)
                pos_v = (playlist_v.index(element) + self.displacement) / len(playlist_v)
                sim += 1 / (abs(pos_u - pos_v) + 1)
        return sim
